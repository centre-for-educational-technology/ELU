...
//Join project form 
 ...
 if($request->join_extra_a2){
  ...
  public function attachGroupPresentation(Request $request) {
    ...
    // Email sending
      ...
      Mail::send('emails.new_poster_notification', ['data' => $data], function ($m) use ($admins_emails) {
        $m->to($admins_emails)->replyTo(getUserEmail(Auth::user()), getUserName(Auth::user()))->subject('Uus portfoolio');
      });
   ...
   * Upload group poster thumbnail
   */
  public function listStudentAnswers (Project $project){

    $members = $project->users()->get()->reject(function ($user) {
      return $user->pivot->participation_role == 'author';
    });


    return view('user.teacher.listStudentAnswers')->with(compact('project', 'members'));
  }
